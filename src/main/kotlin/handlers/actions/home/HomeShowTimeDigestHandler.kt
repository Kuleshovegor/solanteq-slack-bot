...
class HomeShowTimeDigestHandler(di: DI, private val everyWeekTaskService: EveryWeekTaskService) : BlockActionHandler {
    ...
    override fun apply(req: BlockActionRequest, context: ActionContext): Response {
        val response = context.client().viewsOpen { vo ->
            vo.token(token)
                ...
                .view(ModalService.getAllTimeDigestModal(everyWeekTaskService.getAllTimes(teamId)))
        }
